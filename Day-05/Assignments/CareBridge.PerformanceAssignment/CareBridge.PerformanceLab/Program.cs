...
while (true)
{
    ...
    Console.WriteLine("8. Revenue at risk");
    Console.WriteLine("9. Cartesianexplosion");
    Console.WriteLine("10. querySplit");
...
static void RevenueAtRiskDashboard()
{
    ...
    // 1. Projection with GroupBy, no ToList before aggregation
    var claimSummary = db.Claims
        .AsNoTracking() // 2. Disable tracking
        .GroupBy(c => c.Status)
        .Select(g => new
        {
            Status = g.Key,
        ...
        .OrderByDescending(r => r.TotalBilled) // server-side ordering
        .ToList(); // materialize only aggregated rows
    ...
    // 3. Print per-status summary
    foreach (var item in claimSummary)
    {
        Console.WriteLine($"{item.Status,-10} {item.ClaimCount,6} {item.TotalBilled,10} {item.TotalReimbursed,12} {item.Gap,10}");
    }

    Console.WriteLine("------------------------------------------------------------");

    // 4. Compute Revenue-at-Risk (claims not Paid)

    Console.WriteLine($"REVENUE AT RISK (not Paid) : {revenueAtRisk}");

    //change tracking is disabled because just data is getting displayed
    Console.WriteLine("Tracked Entities : " + db.ChangeTracker.Entries().Count());

    // 6. Profiler validation
    Console.WriteLine("SQL Statements (Profiler) : 1 (summary) + 1 (risk figure)");
    Console.WriteLine(
        $"Elapsed Time      : {stopwatch.ElapsedMilliseconds} ms");
}
static void CartesianExplosionDemo()
{
    ...
    // 7. Single eager query with two sibling collections
    var patient = db.Patients
        .AsNoTracking()
        .Where(p => p.Mrn == "MRN999999")
        .Include(p => p.Encounters)
    ...
    // 11. Assert object counts
    var encounterCount = patient.Encounters.Count;
    var diagnosisCount = patient.Encounters.SelectMany(e => e.Diagnoses).Count();
    var claimCount = patient.Encounters.SelectMany(e => e.Claims).Count();
    ...
    Console.WriteLine("Rows returned by SQL : (check Profiler, expect ~900 due to cross-product)");
 ...
 static void SplitQueryDemo()
{
    ...
    // 9. Same eager load but with AsSplitQuery()
    var patient = db.Patients
        .AsNoTracking()
        .Where(p => p.Mrn == "MRN999999")
        .Include(p => p.Encounters)
    ...
    Console.WriteLine("------------------------------------------------");
    Console.WriteLine($"Encounters : {encounterCount} Diagnoses : {diagnosisCount} Claims : {claimCount}");
    Console.WriteLine("SQL Statements (Profiler) : 3 (one per collection)");
    Console.WriteLine("Max rows in any statement : (check Profiler, expect <= 300, no explosion)");
    Console.WriteLine($"Elapsed Time : {sw.ElapsedMilliseconds} ms");
    Console.WriteLine("Tracked entities : " + db.ChangeTracker.Entries().Count());
}
